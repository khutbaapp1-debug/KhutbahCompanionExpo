...
const TARAWIH_KEY = 'ramadan-tarawih-v1';
const QURAN_TOTAL_DAYS = 30;
const PAGES_PER_DAY = 20;

type TarawihData = { date: string; rakats: number };
...
export default function RamadanScreen() {
  ...
  const [tarawihRakats, setTarawihRakats] = useState(0);
  ...
  useEffect(() => {
    AsyncStorage.getItem(TARAWIH_KEY).then((raw) => {
      if (!raw) return;
      const data = JSON.parse(raw) as TarawihData;
      if (data.date === todayKey()) setTarawihRakats(data.rakats);
    });
  }, []);
  ...
  const addTarawih = useCallback(() => {
    setTarawihRakats((prev) => {
      const next = prev + 2;
      void AsyncStorage.setItem(TARAWIH_KEY, JSON.stringify({ date: todayKey(), rakats: next }));
      return next;
    });
  }, []);

  const resetTarawih = useCallback(() => {
    setTarawihRakats(0);
    void AsyncStorage.setItem(TARAWIH_KEY, JSON.stringify({ date: todayKey(), rakats: 0 }));
  }, []);
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 16, color: theme.text, marginBottom: 2 }}>
            Tarawih Tonight
          </Text>
          <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 12, color: theme.textMuted, marginBottom: 20 }}>
            Tap to record each 2 rakats. Resets automatically each day.
          </Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <View>
              <Text style={{ fontFamily: 'Inter_700Bold', fontSize: 52, color: theme.primary, lineHeight: 56 }}>
                {tarawihRakats}
              </Text>
              <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 13, color: theme.textMuted }}>
                rakats prayed
            ...
            <TouchableOpacity
              onPress={addTarawih}
              style={{ backgroundColor: theme.primary, borderRadius: 16, paddingVertical: 16, paddingHorizontal: 24 }}
            >
              <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 16, color: 'white' }}>+2 Rakats</Text>
            </TouchableOpacity>
          </View>
          {tarawihRakats > 0 && (
            <TouchableOpacity onPress={resetTarawih} style={{ alignItems: 'center', marginTop: 14, paddingVertical: 6 }}>
              <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 13, color: theme.textMuted }}>Reset tonight</Text>
            </TouchableOpacity>
          )}
        </View>
      </ScrollView>
